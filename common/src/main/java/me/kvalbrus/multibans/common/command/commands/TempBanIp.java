            ...
            if (length == 2) {
                ...
                try {
                    ...
                    Punishment punishment = super.getPluginManager().getPunishmentManager()
                        .generatePunishment(PunishmentType.TEMP_BAN_IP, player, sender.getName(),
                            date, reason.toString());

                    punishment.activate();

                    ReplacedString listenMessage = new ReplacedString(Message.TEMPBANIP_LISTEN.message)
                    ...
                    ReplacedString creatorMessage = new ReplacedString(Message.TEMPBANIP_CREATOR.message)
                        .replacePlayerName(player);
